...
function loadCartItems() {
    ...
    if (!userData || !isLoggedIn) {
        // Checkout button is disabled and html displays that user needs to be logged in to view cart
    ...
    if (cartItems.length === 0) {
        ...
        cartItems.forEach((item, index) => {
            ...
            // Remove button, item name and price.
            itemDiv.innerHTML = `
                <h4>${item.name}</h4>
            ...
            total += item.price; // Add price of item to total price
        });
